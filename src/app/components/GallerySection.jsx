          ...
          {images.map((img, i) => (
            <motion.button
              ...
              className={`relative overflow-hidden rounded-xl group cursor-pointer ${
                i === 0 || i === 2 ? 'md:row-span-2 h-56 sm:h-64 md:h-full' : 'h-48 sm:h-56 md:h-64'
              }`}
            >
              <img
                src={img.src}
              ...
              {/* <div className="absolute bottom-0 left-0 right-0 p-4 bg-linear-to-t from-slate-900/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <p className="text-white text-sm font-medium">Retreat memory {i + 1}</p>
              </div> */}
            </motion.button>
          ))}
        </div>
